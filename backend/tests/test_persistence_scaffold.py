...
class TestPostgresMigrationSql:
    def test_stateless_db_mode_pg_migration_sql_exists(self):
        migration = (
            ...
            / "migrations"
            / "versions"
            / "20260621_0001_stateless_db_mode_pg.sql"
        )
        ...
        assert "UQ_" not in sql.upper()

    def test_stateless_orm_metadata_does_not_emit_unique_or_foreign_key_constraints(self):
        ...
        for table in (
            CustomAgentRow.__table__,
            MemoryRow.__table__,
            SkillRow.__table__,
            NormalizedSkillRow.__table__,
            SkillFileRow.__table__,
        ):
            assert not [constraint for constraint in table.constraints if isinstance(constraint, UniqueConstraint)]
