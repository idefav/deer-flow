...
from deerflow.artifacts.store import ArtifactPathError, ArtifactStore, make_artifact_store
...
def _is_text_bytes(content: bytes, sample_size: int = 8192) -> bool:
...
def _extract_file_from_skill_archive_bytes(content: bytes, internal_path: str) -> bytes | None:
    try:
...
def _get_runtime_artifact_store(config: AppConfig) -> ArtifactStore | None:
    runtime_storage = getattr(config, "runtime_storage", None)
    if runtime_storage is None:
        return None
    return make_artifact_store(runtime_storage)


def _request_path_to_virtual_path(path: str) -> str:
    return f"/{path.lstrip('/')}"


def _artifact_response_from_bytes(content: bytes, *, filename: str, mime_type: str | None, download: bool) -> Response:
    if download or mime_type in ACTIVE_CONTENT_MIME_TYPES:
        return Response(content=content, media_type=mime_type or "application/octet-stream", headers=_build_attachment_headers(filename))
    ...
    if _is_text_bytes(content):
        try:
            return PlainTextResponse(content=content.decode("utf-8"), media_type=mime_type or "text/plain")
        except UnicodeDecodeError:
            pass

    return Response(content=content, media_type=mime_type, headers={"Content-Disposition": _build_content_disposition("inline", filename)})


def _get_object_artifact(thread_id: str, path: str, *, artifact_store: ArtifactStore, download: bool) -> Response:
    user_id = get_effective_user_id()
    virtual_path = _request_path_to_virtual_path(path)

    try:
        if ".skill/" in virtual_path:
            skill_marker = ".skill/"
            marker_pos = virtual_path.find(skill_marker)
            skill_file_path = virtual_path[: marker_pos + len(".skill")]
            internal_path = virtual_path[marker_pos + len(skill_marker) :]
            archive_content = artifact_store.get_bytes(user_id, thread_id, skill_file_path)
            content = _extract_file_from_skill_archive_bytes(archive_content, internal_path)
            if content is None:
                raise HTTPException(status_code=404, detail=f"File '{internal_path}' not found in skill archive")
            mime_type, _ = mimetypes.guess_type(internal_path)
            return _artifact_response_from_bytes(content, filename=Path(internal_path).name or Path(skill_file_path).stem, mime_type=mime_type, download=download)

        content = artifact_store.get_bytes(user_id, thread_id, virtual_path)
    ...
    return _artifact_response_from_bytes(content, filename=Path(virtual_path).name, mime_type=mime_type, download=download)
...
async def get_artifact(thread_id: str, path: str, request: Request, download: bool = False, config: AppConfig = Depends(get_config)) -> Response:
    ...
    if artifact_store is not None:
        return _get_object_artifact(thread_id, path, artifact_store=artifact_store, download=download)
