...
File editing includes direct writes, string replacement, metadata inspection, directory creation, file copy/move/remove, and Codex-style `apply_patch` edits. Patch hunks use exact context matching; `@@ <context>` anchors pure insertions after the matching line, and `*** End of File` appends at EOF.
