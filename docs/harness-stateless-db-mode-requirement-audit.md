...
1. **Local Docker AIO materialization, bundled provisioner mount-contract consumption, and the remote smoke entrypoint are implemented; executing the remote live smoke remains a rollout gate.** Batch 67 adds a live Docker AIO smoke proving DB context files are written through the sandbox file API, including prompt-exposed `/mnt/skills/...` skill paths backed by a writable per-thread mount. Batch 70 forwards the same `extra_mounts` contract to the remote provisioner create API. Batch 72 makes the bundled provisioner consume that payload and override default mount paths such as `/mnt/skills`. Batch 74 adds an opt-in remote provisioner/K8s smoke test. Batch 91 prevents stale existing Pods from bypassing the current mount contract. Batch 92 preserves provisioner response bodies when remote create fails, so mount-contract rejections are visible in gateway/test output. Batch 93 aligns anonymous remote create payloads with the bundled provisioner's string `thread_id` validation. Batch 94 adds a preflight CLI for live-gate environment checks and exact command reporting. Batch 111 adds JSON evidence-file output for preflight/run results, Batch 112 adds evidence generation time and cwd metadata, Batch 113 adds git source metadata, Batch 114 makes that source-metadata evidence coverage portable to no-`.git` test environments, Batch 115 adds an explicit evidence `schema_version`, Batch 116 adds checked env names without values, Batch 117 adds evidence-file validation for archived gate records, Batch 118 adds strict `--require-run` validation so preflight-only evidence is not accepted as sign-off, Batch 119 rejects contradictory success evidence when a recorded execution has a non-zero `exit_code`, Batch 120 adds optional stdout/stderr log capture for executed gate commands, Batch 121 can require those referenced log files during evidence validation, Batch 122 resolves relative log paths from the evidence JSON directory for portable evidence bundles, Batch 123 makes generation write relative log dirs next to the evidence file while recording relative paths, Batch 124 rejects strict evidence whose execution command does not match the preflight command for the selected gate, Batch 125 records and validates SHA-256/byte counts for archived stdout/stderr logs, Batch 126 rejects empty selected-gate lists, unknown gates, not-ready top-level preflight, and selected gates whose `ready_to_invoke` state is not true in strict run evidence, Batch 127 rejects unselected or duplicate execution records, and Batch 128 rejects duplicate selected/preflight gate entries. The remote smoke still must be run in a real deployment to prove cluster permissions and node path mapping.
...
3. **Live client model-config gate**
   - Full backend verification now passes in the no-model environment: `4874 passed, 36 skipped, 12 warnings`.
