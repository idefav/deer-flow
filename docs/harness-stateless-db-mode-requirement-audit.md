...
- Runtime PVC removal is locally closed behind `runtime_storage.backend=object`: runtime workspace/uploads/outputs/ACP files use S3-compatible object storage, AIO sandboxes materialize/flush/reconcile deletions through the file API with materialize budgets, all tracked sandbox teardown paths attempt best-effort flush, ACP gateway staging remains compatibility-only, ACP sandbox-native mode now runs subprocesses in named isolated ephemeral AIO sandboxes, object-mode sandbox-native ACP fails closed without `thread_id`, ACP workspace flush refreshes the active leader sandbox with `/mnt/acp-workspace` only, object-mode local containers no longer report thread-data mounts, upload conversion parity is covered for gateway and embedded client, the bundled provisioner omits `/mnt/user-data`, provisioner reserved labels cannot be overridden by create-option labels, object-mode sandbox creation uses PostgreSQL advisory locks, and `runtime_object_storage` / `acp_sandbox_native` preflights fail closed unless backend config, AIO provider selection, ACP execution mode, and live-gate opt-in are aligned.
...
1. **Runtime PVC removal is locally closed, but target-environment live evidence is still required.** The code now supports object-backed runtime artifacts, migration from existing `.deer-flow`/PVC files, sandbox materialize/flush/delete reconcile, materialize budgets, lifecycle flush hardening, ACP sandbox-native isolated ephemeral execution, immediate active-leader ACP workspace refresh after object-store flush, fail-closed object-mode ACP calls without `thread_id`, gateway/client/channel/tool object reads/writes, upload conversion parity, provisioner PVC removal, reserved-label protection, and `runtime_object_storage` / `acp_sandbox_native` gates. The remote/object/ACP/model smoke still must be run in a real deployment with `RUNTIME_STORAGE_BACKEND=object`, no `USERDATA_PVC_NAME`, configured object storage, sandbox-native ACP config, and archived evidence to prove cluster/provisioner behavior and end-to-end runtime artifact flow.
...
4. **Final backend verification is current with live-client auto-skip in unconfigured environments.** `uv --directory backend run pytest -q` passes in this workstation with model-backed live tests skipped because local `config.yaml` has no configured models. Batch 95 makes the live-gate preflight report that no-model state as `config_issues` for `requires_llm`; Batch 96 makes the same preflight respect DB config source when `DEER_FLOW_CONFIG_SOURCE=db`; Batch 97 reports missing `$ENV_NAME` model credential references as `missing_env`; Batch 104 makes the direct `tests/test_client_live.py` entrypoint preload DB AppConfig through the same lower-level bootstrap DB loader used by gateway startup before deciding whether to skip; Batch 105 makes file-mode preflight honor `DEER_FLOW_CONFIG_PATH`, matching `AppConfig.resolve_config_path()`; Batch 106 makes DB-mode preflight use the same bootstrap sqlite/Postgres URL parser as runtime startup; Batch 107 makes `test_client_e2e.py` and `test_create_deerflow_agent_live.py` use the shared readiness helper and active AppConfig; Batch 108 makes `test_client_live.py` use the same helper before preloading active config; Batch 109 makes file-mode preflight honor `DEER_FLOW_PROJECT_ROOT/config.yaml` when no explicit config path is set. Current full regression: `4969 passed, 37 skipped, 12 warnings`. `uv --directory backend run ruff check .` passes, and `git diff --check` is clean.
...
| Runtime PVC is removed for sandbox workspace/uploads/outputs/ACP files | **Verified by focused coverage / Full local regression / ACP artifact visibility gate command / Pending production live evidence** | `RuntimeStorageConfig`, `S3ArtifactStore`, gateway upload/artifact object-mode tests, sandbox materializer delete-reconcile and budget tests, AIO provider object-mode mount/advisory-lock/lifecycle flush tests, ACP sandbox-native provider/runner tests, ACP active-leader refresh coverage, object-mode missing-`thread_id` fail-closed coverage, ACP preflight AIO-provider false-ready coverage, provisioner object-runtime/reserved-label tests, uploads/thread-data/present/view-image middleware/tool object tests, channel manager/Feishu object file tests, embedded client object tests including upload conversion parity, runtime artifact import tests, `runtime_object_storage` and `acp_sandbox_native` live-gate preflights, opt-in `tests/test_acp_sandbox_native_live.py`, multi-gate evidence bundle coverage, and full backend regression `4969 passed, 37 skipped, 12 warnings` | Run `scripts/import_runtime_artifacts_to_object_store.py`, then run `DEER_FLOW_RUN_ACP_SANDBOX_NATIVE=1 uv --directory backend run python scripts/check_stateless_live_gates.py --gate runtime_object_storage --gate acp_sandbox_native --gate remote_live --gate requires_llm --run --evidence-path <bundle>/evidence.json --evidence-log-dir logs` in the target environment and validate it with `--validate-evidence <bundle>/evidence.json --require-run --require-logs`. |
...
3. **Live client model-config gate**
   - Full backend verification now passes in the no-model environment: `4969 passed, 37 skipped, 12 warnings`.
