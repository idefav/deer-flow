...
| Runtime PVC is removed for sandbox workspace/uploads/outputs/ACP files | **Verified by focused coverage / Static object-storage gate / Remote live-smoke execution gate** | `RuntimeStorageConfig`, `S3ArtifactStore`, gateway upload/artifact object-mode tests, sandbox materializer tests, AIO provider object-mode mount/advisory-lock tests, provisioner object-runtime gate tests, uploads/thread-data/present/view-image middleware/tool object tests, channel manager/Feishu object file tests, embedded client object tests, runtime artifact import tests, and `runtime_object_storage` live-gate preflight | Run `scripts/check_stateless_live_gates.py --gate runtime_object_storage --json`, migrate existing runtime files with `scripts/import_runtime_artifacts_to_object_store.py`, then run remote/model evidence gates in the target environment. |
